...
namespace kkAsync
{
    public class Class2
    {
        ...
        {
            internal ISubject<Progress<byte[]>>  x; //yを見張る、購読可能
            internal IObserver<Progress<byte[]>> w;　//yに渡す分 と 
            internal HttpWebRequest i;
            internal IObservable<Progress<byte[]>> y; //HttpWebRequestの分
            public bool isRunning = false;
            public Edd(HttpWebRequest req)
            {
                i = req;
                w = Observer.Create<Progress<byte[]>>(s => { x.OnNext(s); byteCount += s.Value.Length; System.Diagnostics.Debug.Assert(byteCount == s.BytesReceived); }, (ex) => x.OnError(ex), () => x.OnCompleted()); //そのまますべて転送するObserverを定義
                x = new Subject<Progress<byte[]>>();
            ...
            //    this.Dispose(true);
            //}
            private IObservable<Progress<byte[]>> observableInstance; //使う側が読むやつ

            public IObservable<Progress<byte[]>> myObservable
            {
